...
from app.core.langgraph.agents.insurance import PLAN_SURFACE_PREFIX, build_insurance_agent
...
def _build_insurance(plan_loader):
...
async def test_query_node_writes_envelope_and_prompt_reports_count():
    agent, model = _build_insurance(lambda: [_row(), _row(group_code="G0208")])
...
async def test_each_turn_gets_unique_surface_id():
    agent, _ = _build_insurance(lambda: [_row()])
...
async def test_no_rows_degrades_to_text():
    agent, model = _build_insurance(list)
...
def _build_supervisor(plan_loader):
...
async def test_sse_emits_fence_before_stop_and_persists_it(monkeypatch):
    ...
    graph = _build_supervisor(lambda: [_row(), _row(group_code="G0208")])
    session = FakeSession()
