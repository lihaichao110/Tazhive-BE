...
down_revision: str | Sequence[str] | None = "bbb7a18d70ba"
...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            ...
            nullable=True,
            comment="产品说明文档 PDF 链接（可空）",
        ),
        sa.Column(
            "additional_premium_rule_url",
            sa.String(length=512),
            nullable=True,
            comment="产品追加保险费规则 PDF 链接（可空）",
        ),
        sa.Column(
            "classification", sa.String(length=10), nullable=False, comment="产品分类分级 P1/P2/P3"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    op.create_index(
        op.f("ix_products_classification"), "products", ["classification"], unique=False
    )
